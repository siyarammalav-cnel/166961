	  ...
	  /**
		...
		public function print_script() {
			...
			if ( 'enabled' != self::$file_generation || self::$fallback_js ) {
				if ( !is_null( self::$script ) || ( '' != self::$script ) ) {
					echo '<script type="text/javascript" id="ive-script-frontend">' . stripslashes( str_replace( '\n', '', html_entity_decode( esc_js( self::$script ) ) ) ) . '</script>';
				}
			}
			...
			if ( 'enabled' != self::$file_generation || self::$custom_fallback_js ) {
				...
				if ( self::$can_enqueue_custom_assets == true ) {
					if ( !is_null( self::$custom_script ) || ( '' != self::$custom_script ) ) {
						echo '<script type="text/javascript" id="ive-custom-script-frontend">' . stripslashes( str_replace( '\n', '', html_entity_decode( esc_js( self::$custom_script ) ) ) ) . '</script>';
					}
				}
			}
		...
		public function print_stylesheet() {
			...
			if ( 'enabled' != self::$file_generation || self::$fallback_css ) {
				if ( !is_null( self::$stylesheet ) || ( '' != self::$stylesheet ) ) {
					ob_start();
					?>
					<style id="ive-style-frontend"><?php echo html_entity_decode( esc_html( self::$stylesheet ) ); ?></style>
					<?php
					ob_end_flush();
				}
			}
			...
			if ( 'enabled' != self::$file_generation || self::$custom_fallback_css ) {
				...
				if ( self::$can_enqueue_custom_assets == true ) {
					if ( !is_null( self::$custom_stylesheet ) || ( '' != self::$custom_stylesheet ) ) {
						ob_start();
						?>
						<style id="ive-custom-style-frontend"><?php echo html_entity_decode( esc_html( self::$custom_stylesheet ) ); ?></style>
						<?php
						ob_end_flush();
					}
				}
		...
		public function get_block_css_and_js( $block ) {
			...
			switch ( $name ) {
				...
				case 'ive/carousel':
					...
					break;
				case 'ive/slide':
						$css += IVE_Block_Helper::get_slider_css( $blockattr, $block_id );
						break;
				...
				case 'ive/form':
					$css += IVE_Block_Helper::get_form_css( $blockattr, $block_id );
					break;
				...
				case 'ive/button-single':
					...
					IVE_Block_JS::get_form_button_gfont( $blockattr );
					break;

				case 'ive/ive-productscarousel':
					$css += IVE_Block_Helper::get_productscarousel_css( $blockattr, $block_id );
					$js  .= IVE_Block_JS::get_productscarousel_js( $blockattr, $block_id );
					IVE_Block_JS::get_productscarousel_gfonts( $blockattr );
					break;
				default:
					// Nothing to do here.
					break;
		...
		public function frontend_gfonts() {
			...
			if ( isset( $link ) && ! empty( $link ) ) {
				...
				wp_enqueue_style( 'ive-enqueue-google-fonts', '//fonts.googleapis.com/css?family=' . esc_attr( str_replace( '|', '%7C', $link ) ), array(), IVE_VER, 'all' );
			}
		}
	}
