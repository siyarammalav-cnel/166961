...
function bk_widgets_backend_enqueue()
{
    ...
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'autoparts-online-store' ),
 ...
 *
 * Class Autoparts_Notice_Handler
 */
class Autoparts_Notice_Handler {
	...
	public function bk_getting_started() {
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'autoparts-online-store' );
        ...
        if( $request == 2 && strpos( $slug, 'autoparts-online-store' ) === false ){
            wp_send_json_error( );
        }
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'autoparts-online-store' );
