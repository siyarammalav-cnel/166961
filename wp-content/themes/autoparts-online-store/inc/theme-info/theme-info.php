 ...
 */
if ( ! class_exists( 'Autoparts_Theme_Info' ) ) {
    class Autoparts_Theme_Info{
        ...
        public function __construct( $config ) {
            ...
            add_action( 'wp_ajax_autoparts_theme_info_update_recommended_action', array( $this, 'update_recommended_action_callback' ) );
        }
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'autoparts-online-store' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Theme', 'autoparts-online-store' );
         ...
         */
        public function autoparts_theme_info_screen() {
            ...
            if (
                ...
                echo '<a href="' . esc_url( 'https://www.templatehouse.net/product/auto-parts-wordpress-theme/' .$theme_name_config_url ).'" target="_blank">';
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'autoparts-online-store' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'autoparts-online-store' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'autoparts-online-store' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'autoparts-online-store' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'autoparts-online-store').'</a>';
                }
                echo '</div>';
            }
        }
        ...
        public function call_plugin_api( $slug ) {
            ...
            if ( false === ( $call_api = get_transient( 'autoparts_theme_info_plugin_information_transient_' . $slug ) ) ) {
                ...
                set_transient( 'autoparts_theme_info_plugin_information_transient_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
            }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'autoparts-online-store' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview', 'autoparts-online-store' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<thead>';
                    echo '<tr>';
                    echo '<th>' . esc_html__( 'Theme Features','autoparts-online-store' ) . '</th>';
                    echo '<th>' . esc_html__( 'Free','autoparts-online-store' ) . '</th>';
                    echo '<th>' . esc_html__( 'Pro (Premium)','autoparts-online-store' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    echo '<td></td>';
                    echo '<td></td>';
                    echo '<td><a href="' . esc_url( 'https://www.templatehouse.net/product/auto-parts-wordpress-theme/') .'" target="_blank" class="button button-primary button-hero">Buy Pro</a></td>';
                    echo '</tr>';
                    echo '</tbody>';
                    echo '</table>';
                    echo '</div>';
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {
                ...
                $count = $this->count_recommended_actions();
                wp_localize_script( 'bk-theme-info-js', 'autoparts_theme_info_object', array(
...
$config = array(
    ...
    // Main welcome title
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'autoparts-online-store' ), $theme_name_config ),

    // Main welcome content
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'autoparts-online-store' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Info', 'autoparts-online-store' ),
            'url' => esc_url( 'https://www.templatehouse.net/product/auto-parts-wordpress-theme/' )
        ),
        'pro_url'  => array(
            'text' => __( 'Buy Pro', 'autoparts-online-store' ),
            'url' => esc_url( 'https://www.templatehouse.net/product/auto-parts-wordpress-theme/' )
        ),
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'autoparts-online-store' ),
        'free_pro'             => esc_html__( 'Compare Free Vs Pro', 'autoparts-online-store' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'autoparts-online-store' ),
    ...
    'gs_steps' => array(
        ...
        'two' => array(
            'title' => esc_html__( 'Pre-built Demos', 'autoparts-online-store' ),
            'desc' => esc_html__( 'We recommend utilizing our collection of pre-designed starter site templates as an efficient and convenient way to begin your project.', 'autoparts-online-store' ),
            'button_label' => esc_html__( 'Browse Demos', 'autoparts-online-store' ),
            'button_link' => esc_url( 'https://templatehouse.net/autoparts/' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        ...
        'seven' => array(
            'title' => esc_html__( 'Need WordPress Theme Developemnt or Customization?', 'autoparts-online-store' ),
            'desc' => esc_html__( 'We Create Beautiful & SEO Friendly WordPress Websites. Theme Customization to Custom WordPress Plugin Development.', 'autoparts-online-store' ),
            'button_label' => esc_html__( 'Contact Now', 'autoparts-online-store' ),
        ...
        'five' => array(
            'title' => esc_html__( 'Love Our Product?', 'autoparts-online-store' ),
            'desc' => esc_html__( 'We would greatly appreciate your motivation through a 5-star rating on our profile.', 'autoparts-online-store' ),
            'button_label' => esc_html__( 'Rate Us', 'autoparts-online-store' ),
            'button_link' => esc_url( 'https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'six' => array(
            'title' => esc_html__( 'Have Question? Ask Us, Now!', 'autoparts-online-store' ),
            'desc' => esc_html__( 'We are committed to resolving any issues that may arise to ensure a seamless experience while using our theme.', 'autoparts-online-store' ),
            'button_label' => esc_html__( 'Get Support', 'autoparts-online-store' ),
    ...
    'free_pro' => array(
        array(
            'desc'=> __( 'Access to All Starter Sites', 'autoparts-online-store' ),
            'free' => __( 'no','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ), 
        array(
            'desc'=> __( 'Access to All Templates & Patterns', 'autoparts-online-store' ),
            'free' => __( 'no','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Work with the Page Builders', 'autoparts-online-store' ),
            'free' => __( 'no','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Creative & Unique Blocks', 'autoparts-online-store' ),
            'free' => __( 'no','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Elementor Ready', 'autoparts-online-store' ),
            'free' => __( 'no','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Priority Updates', 'autoparts-online-store' ),
            'free' => __( 'no','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Priority Support', 'autoparts-online-store' ),
            'free' => __( 'no','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Full Site Editing', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Customize Everything', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Lighting Fast', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'One-click Demo Installer', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Fully Responsive', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Clean & Optimized Code', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Multiple Site Styles', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Global Settings', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'CSS Animation', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Gutenberg Ready', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'eCommerce Ready', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'SEO Optimized', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'Multilingual Compatibility', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
        array(
            'desc'=> __( 'RTL/LTR Language Support', 'autoparts-online-store' ),
            'free' => __( 'yes','autoparts-online-store' ),
            'pro'  => __( 'yes','autoparts-online-store' ),
        ),
    ),
...
return new Autoparts_Theme_Info( $config );